...
sortList=[];
counterArr=[];
CalculateObj(obj:imageObject){
return (obj.X2-obj.X1)*(obj.Y4-obj.Y1);//width*height
}
changeListObjFromBigToSmallest(){
for (let i = 0; i < this.listObj.length; i++)
 this.counterArr[i]=this.CalculateObj(this.listObj[i]);
var maxObj;
for (let i = 0; i < this.listObj.length; i++) {
  maxObj=i;
  for (let j = 0; j < this.listObj.length; j++) {
   if(this.counterArr[j]>this.counterArr[i])//we need to change the maximum obj
        maxObj=j;
  }
  //push the max obj to sort list , and change the counterArr
this.sortList.push(this.listObj[maxObj]);
this.counterArr[maxObj]=-1;
}
...
//saves the object name
  saveObj(){
    ...
    if(this.currentobject!=null){
      ...
      debugger;
      document.getElementById(this.mid).innerHTML=this.currentobject.Name;
      document.getElementById(this.mid).style.textAlign="center";
      document.getElementById(this.mid).style.fontSize="20px";
      // document.getElementById(this.mid).style.fontSize="3px";
  ...
  removeObj(){//obj:imageObject
    this.showradio=false;
    this.mid//the index of my object
    let index=this.mid;
    ...
    //remove from div!
    document.getElementById(this.mid).innerHTML="";
  }
  // saveObjOther(){
  //   debugger;
  //   this.currentobject.Name=this.valuetext;
...
file:File;
        ...
        resolve(
          //img ,id ,cat id
           this.userserv.getListObject(2,this.imageBase64).then(data => {
             ...
             this.spinner=false;
            debugger;
             document.getElementById("image").setAttribute("style","margin-top: -8%;max-width:55%;border: 2px;border-radius: 20%;margin-left: 23%;border:solid;border-width: 3px");
             this.viewlist=true;
          ...
          //list Save imageBase64
           this.userserv.postListObject(2,this.imageBase64,this.listsave).then(data => {
            ...
            if(data!=false)
              ...
              this.notsaved=true;
            this.spinner=false;
            document.getElementById("image").setAttribute("style","margin-left:23%;max-width: 55%;border: 2px;border-radius: 20%;border:solid;border-width: 5px;margin-top: -8%;");
            debugger;
            this.picturetaken=false;
            console.log(this.listObj);
           })
  ...
  async sendToVision() {
    debugger;
   this.listFull=false;
    document.getElementById("image").setAttribute("style","max-width: 55%;border: 2px;border-radius: 20%;margin-left:23%;border:solid;border-width: 3px;opacity:0.4;margin-top:-8%;");
    this.listObj=[];
    this.spinner=true;
 ...
 imageBase64;
  ...
  //send objectlist to server and update in db
    saveInDB(){
      ...
      document.getElementById("image").setAttribute("style","margin-left:23%;max-width:55%;border: 2px;border-radius: 20%;opacity:0.4 ;border:solid;border-width: 5px;margin-top: -8%;");
...
/////////
  ...
  document.getElementById(Id).setAttribute("style","text-align:center;border-radius: 21px;position:absolute;height:"+this.sizeH+"px; width:"+this.sizeW+"px;left:"+this.locLeft+"px;top:"+this.locTop+"px;border:3px solid black; background-color:transparent;font-size:18px;font-weight: 900;");
}
currentobject;
mid;
exist=false;
makeTransparency(event,obj:imageObject){
  this.exist=false;
  for (let i = 0; i < this.listsave.length; i++) {
    if(this.listsave[i].ObjectId==obj.ObjectId)
    this.exist=true;
  }
  this.ichecked=='radio';
  this.valuetext="Enter Object Name";
  this.sendtodb=false;
  document.getElementById("image").setAttribute("style","max-width:55%;border: 2px;border-radius: 20%;margin-left:23%;border:solid;border-width: 3px;margin-top: -8%;");
  debugger;
  this.showradio=true;
///
  ...
  this.mid=i;//the div that we need -clossestObj!
  ...
  if(this.mid!=null){
   ...
   //console.log(document.getElementById(mid).getAttributeNames());
     var offTop= document.getElementById(this.mid).offsetTop;
    this.forObject();//all the element dont change, l'm here:
    var size= document.getElementById(this.mid).getBoundingClientRect();
    document.getElementById(this.mid).setAttribute("style","border-radius:21px;position:absolute;height:"+size.height+"px; width:"+size.width+"px;left:"
    +size.left+"px;top:"+offTop+"px;border:3px dashed black;background-color: #fff; opacity:0.3;text-align:center;font-size:18px;font-weight: 900;");
    }
